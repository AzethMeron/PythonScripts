...
if __name__ == '__main__':
    ...
    max_len = 6
    threads = 12
    cal = calculate_number_of_passwords(min_len, max_len, charset)
    passnum = threads * 100000
    generator = gen_passwords(min_len, max_len, charset)
    r = math.ceil(cal/passnum)+1
    for i in range(1, r):
        print(f'Processing bulk {i}/{r}')
        msg = Client.start("a.zip", get_passwords(generator, passnum), ".tmp", threads, 0)
        if msg:
            print(msg)
            break
